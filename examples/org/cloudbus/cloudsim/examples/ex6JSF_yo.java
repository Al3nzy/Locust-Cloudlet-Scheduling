...
import org.cloudbus.cloudsim.DatacenterBroker_jsf;
...
public class ex6JSF_yo {
	...
	private static List<Vm> createVM(int userId, int vms) {
		...
		for(int i=0;i<vms;i++){
			vm[i] = new Vm(i, userId, mips[i], pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared());
			
			//vm[i] = new Vm(i, userId, mips[i], pesNumber, ram, bw, size, vmm[i], new CloudletSchedulerSpaceShared());
	...
	private static List<Cloudlet> createCloudlet(int userId, int cloudlets){
		...
		for(int i=0;i<cloudlets;i++){
			Random rojb = new Random(); // to create Random behaviour for my cloudlet 
			cloudlet[i] = new Cloudlet(i,  length + rojb.nextInt(99000) , pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
	...
	public static void main(String[] args) {
		...
		try {
			...
			@SuppressWarnings("unused")  // IN CASE I WANT 2 OR MORE DATACENTER
			Datacenter datacenter1 = createDatacenter("Datacenter_1");

			
			
			//Third step: Create Broker
			DatacenterBroker_jsf broker = createBroker();
	...
	private static DatacenterBroker_jsf createBroker(){

		DatacenterBroker_jsf broker = null;
		try {
			broker = new DatacenterBroker_jsf("Broker");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private static void printCloudletList(List<Cloudlet> list) {
		...
		Log.printLine("Cloudlet ID" + indent + "STATUS" + indent +
				"Data center ID" + indent + "VM ID" + indent +"length" + indent + indent + "Time" + indent + "Start Time" + indent + "Finish Time");
		...
		try {
			...
			FileOutputStream fileOut = new FileOutputStream("D://Result_JSF.xls");
	...
	//		cellStyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
			...
			for(int i=0;i<size;i++){  // loop for printing
				HSSFRow row2 = worksheet.createRow(i+1); //  Row no.
				cloudlet = list.get(i);
					
										
					HSSFCell cellA = row2.createCell((short) 0); // colume no.					
...
/*static void drow(){
		...
		double y[]= {10,32,4,1,9,97,5,54,89};
		
		System.out.println();
		final Graph graph = new Graph("5G wireless network",x,y);
