 ...
 */
public class DatacenterBroker_jsf extends SimEntity {
	 ...
	 * @post $none
	 */
	public DatacenterBroker_jsf(String name) throws Exception {
		super(name);
	...
	protected void submitCloudlets() {
		...
		List<Cloudlet> tempList = new ArrayList<Cloudlet>();
		...
		for(Cloudlet checkCloudlet2: tempList)
		{	
			// here to check which thw first cloudlet shoud i take f
			if(smallestCloudlet.getCloudletLength()>checkCloudlet2.getCloudletLength())
			{
				smallestCloudlet=checkCloudlet2;
		...
		{
		Log.printLine(  "Cloudlet ID: " + printCloudlet.getCloudletId() + " , Cloudlet Length:" + printCloudlet.getCloudletLength());
		...
		/*for (int i=0;i<totalCloudlets;i++);
		{
			...
			{	
				Log.printLine(checkCloudlet.getCloudletLength() + "----------------------");
			...
			tempList.remove(smallestCloudlet);
			smallestCloudlet=tempList.get(0);
		}
		*/
		int count=1;
		for(Cloudlet printCloudlet : sortList)
